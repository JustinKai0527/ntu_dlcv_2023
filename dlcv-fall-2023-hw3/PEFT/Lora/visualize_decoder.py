...
#         self.mha = nn.MultiheadAttention(embed_dim=cfg.n_embd, num_heads=cfg.n_head, batch_first=True, kdim=1280, vdim=1280)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, enc_feat):
        ...
        att = F.softmax(att, dim=-1).mean(dim=1)                                             # B, H, S, T
...
class Block(nn.Module):
    ...
    def forward(self, x, enc_feat):
        ...
        out, att_map = self.cross_attn(self.ln_cross(x), enc_feat)
        x = x + out
...
class Decoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h = nn.Sequential(*[Block(cfg, adapter_exist=True) for _ in range(cfg.n_layer)]),
            ln_f = nn.LayerNorm(cfg.n_embd)
        ))
        self.lm_head = lora.Linear(cfg.n_embd, cfg.vocab_size, r=8, bias=False)
    ...
    def forward(self, x: Tensor, enc_feat: Tensor):
        ...
        for i in range(self.cfg.n_layer):
            x, att_map = self.transformer.h[i](x, enc_feat)
