...
class CLIPScore:
    ...
    def __call__(self, predictions, images_root):
        ...
        for img_name, pred_caption in predictions.items():
            ...
            if clip_max < score:
                clip_max = score
                clip_max_file = img_name
            elif clip_min > score:
                clip_min = score
                clip_min_file = img_name

            total_score += self.getCLIPScore(image, pred_caption)
        return total_score / len(predictions), (clip_max_file, clip_max), (clip_min_file, clip_min)
...
def main(args):
    ...
    # CLIPScore
    clip_score, top1_img, last1_img = CLIPScore()(predictions, args.images_root)
    print(*top1_img, *last1_img)
    print(clip_score)
