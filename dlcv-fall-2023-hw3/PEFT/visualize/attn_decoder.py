...
class CrossAttention(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.mha = nn.MultiheadAttention(embed_dim=cfg.n_embd, num_heads=cfg.n_head, batch_first=True, kdim=1024, vdim=1024)
    ...
    def forward(self, x, enc_feat):
        ...
        # print(attn_out_weights.shape)
        # return attn_out, attn_out_weights
        return attn_out, attn_out_weights
# class CrossAttention(nn.Module):
...
#         self.cfg = cfg
#         self.enc_dim_reduc = nn.Linear(1280, 512)
#         self.enc_attn = nn.Linear(512, 2 * cfg.n_embd)
#         self.dec_attn = nn.Linear(cfg.n_embd, cfg.n_embd)
#         self.c_proj = nn.Linear(cfg.n_embd, cfg.n_embd)
#         self.n_head = cfg.n_head
#         self.n_embd = cfg.n_embd
...
#         B, T, C = x.size() # batch, context, embedding
#         S = enc_feat.shape[1]
#         enc_feat = self.enc_dim_reduc(enc_feat)
#         k, v = self.enc_attn(enc_feat).split(self.cfg.n_embd, dim=2)# B, S, C
#         q = self.dec_attn(x)                                        # B, T, C

#         # get att N, T, S
#         att_weight = F.softmax((q @ k.transpose(-2, -1) * (1.0 / math.sqrt(k.size(-1)))), dim=-1)
        
#         k = k.view(B, S, self.n_head, C // self.n_head).transpose(1, 2)          # B, H, S, C // H  
#         q = q.view(B, T, self.n_head, C // self.n_head).transpose(1, 2)          # B, H, T, C // H
#         v = v.view(B, S, self.n_head, C // self.n_head).transpose(1, 2)          # B, H, S, C // H
#         att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))          # (B, H, S, C//H) @ (B, H, C//H, T)  B, H, S, T
        
#         att = F.softmax(att, dim=-1)                                             # B, H, S, T
#         return self.c_proj((att @ v).transpose(1, 2).contiguous().view(B, T, C)), att_weight
...
class Block(nn.Module):
    ...
    def forward(self, x, enc_feat):
        ...
        if self.adater_exist:
            out, attn_map = self.cross_attn(self.ln_cross(x), enc_feat)
            x = x + self.adapter_attn(out)
...
class Decoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h = nn.Sequential(*[Block(cfg, adapter_exist=False) for _ in range(cfg.n_layer)]),
    ...
    def forward(self, x: Tensor, enc_feat: Tensor):
        ...
        for i in range(self.cfg.n_layer):
            if i == self.cfg.n_layer-1:
                x, attn_map = self.transformer.h[i](x, enc_feat)
            else:
                x, _ = self.transformer.h[i](x, enc_feat)
