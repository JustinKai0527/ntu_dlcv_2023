...
if __name__ == "__main__":
    ...
    decoder = Decoder(cfg)
    decoder.load_state_dict(torch.load("decoder/model_1.pt"), strict=False)
    ...
    with torch.no_grad():
        ...
        for img, file in tqdm(dataloader):
            ...
            img = np.array(transforms.Resize((224, 224))(Image.open(file[0])))
            for i in range(len(text)):
                
                map = attention_map[i].reshape(1, 16, 16)
                map = nn.functional.interpolate(map.unsqueeze(0), scale_factor=14, mode='bilinear')[0].detach().cpu().numpy().reshape(224, 224, 1)
                axes[i // 10, i % 10].imshow(img)
                axes[i // 10, i % 10].imshow(map, alpha=0.6, cmap='rainbow')
                axes[i // 10, i % 10].set_xlabel(f'{text[i]}')
                
            # Remove empty subplots
            for i in range(len(text), 2 * 10):
